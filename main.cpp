...
namespace {
	...
	void TestSize() {
		...
		ASSERT_EQUAL(sheet->GetPrintableSize(), (Size{ 5, 1 }));
		...
		sheet->SetCell("D9"_pos, "");
		sheet->SetCell("A1"_pos, "");
		ASSERT_EQUAL(sheet->GetPrintableSize(), (Size{ 9, 4 }));
		sheet->ClearCell("D9"_pos);
